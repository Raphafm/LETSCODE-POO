...
public class ToInvest {
    public static void run(List<Investment> listInvestment) {
        for (int i = 0; i < listInvestment.size(); i++) {
            System.out.printf("%d - %s %s %s %n",i+1, listInvestment.get(i).getDateInvested(),
                    listInvestment.get(i).getProduct().getName(), listInvestment.get(i).getProduct().getProductType());
